...
//
// Deliberately NOT sent: the feed name and the feed category. Sites rendered the feed name
// in their author slot, so articles were bylined "RSS Feed" or "Security.nl", and the raw
// feed category turned into a shouty "COMPANION" tag. The original is still credited
// through `sourceUrl`, which is a link rather than a byline.
export function buildArticlePayload(a) {
  const bodyHtml =
  ...
  return {
    ...
    faq: a.faq || null,
    // 'news' | 'longread' — lets a site give deep dives their own card treatment.
    // Sites that don't know the field simply ignore it, so this stays backwards compatible.
