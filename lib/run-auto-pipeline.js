...
async function runAutoPipeline({ force = false, fetchOnly = false, singleAutomationId = null } = {}) {
  ...
  for (const automation of enabled) {
    ...
    const relevanceOf = (a) => (a.matchedKeywords ? a.matchedKeywords.length : 0)
    ...
    if (instantPublish) {
      if (prioritizeRecency) {
        ranked = [...newArticles].sort((a, b) => byRecency(a, b) || byRelevance(a, b))
      } else {
        const fresh = newArticles.filter(a => ageHoursOf(a) <= MAX_FRESH_HOURS)
        const pool = fresh.length > 0 ? fresh : newArticles
