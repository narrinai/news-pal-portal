...
const { DEFAULT_MIN_RELEVANCE } = require('./relevance-gate')
...
async function generateLongread(automationId, { force = false, language = null } = {}) {
  ...
  // A story the editorial gate judged off-topic for this site is no basis for a 3000-word
  // piece either, so it never reaches the dossier. Items from before the gate existed (no
  // score) are still eligible.
  let longreadAiSettings = {}
  try { if (automation.ai_settings) longreadAiSettings = JSON.parse(automation.ai_settings) } catch {}
  const minRelevance = Number.isFinite(Number(longreadAiSettings.min_relevance))
    ? Number(longreadAiSettings.min_relevance)
    : DEFAULT_MIN_RELEVANCE

  const candidates = all
    .filter(a => a.article_type !== 'longread')
    .filter(a => a.title && a.url)
    .filter(a => typeof a.relevance_score !== 'number' || a.relevance_score < 0 || a.relevance_score >= minRelevance)
    .filter(a => {
