...
async function runAutoPipeline({ force = false, fetchOnly = false, singleAutomationId = null } = {}) {
  ...
  for (const automation of enabled) {
    ...
    // Instant mode picks the MOST SUITABLE-for-this-site fresh article: blend relevance
    // (site/tag keyword matches) with recency, so it favours on-topic news from the last
    // few days rather than just the newest item. Scheduled mode ranks purely by relevance.
    const instantScore = (a) => {
      const relevance = a.matchedKeywords ? a.matchedKeywords.length : 0
      const ageHours = (Date.now() - new Date(a.publishedAt || 0).getTime()) / 3600000
      const recency = Math.max(0, 72 - ageHours) // 0..72, higher = fresher (last 3 days)
      return relevance * 24 + recency
    }
    const ranked = instantPublish
      ? [...newArticles].sort((a, b) => instantScore(b) - instantScore(a))
      : newArticles
