...
const { findRelatedArticles } = require('./article-clustering')
const { assessRelevance, DEFAULT_MIN_RELEVANCE } = require('./relevance-gate')
...
async function runAutoPipeline({ force = false, fetchOnly = false, singleAutomationId = null } = {}) {
  ...
  for (const automation of enabled) {
    // instant_publish automations must run EVERY hour (they pace themselves toward
    // articles_per_day across the day); only the once-a-day scheduled mode is gated to
    // a single pipeline_hour.
    if (!force && !automation.instant_publish) {
    ...
    // Parsed once here (not per article) because the relevance gate below needs it too.
    let aiSettings = {}
    ...
    const instantCandidates = instantPublish
      ? (() => {
          ...
          const backlog = existingArticles.filter(a =>
            ...
            // Already folded into another article — publishing it too would duplicate the story.
            !a.merged_into)
    ...
    // Editorial gate: keyword/feed matching is coarse enough that plainly off-topic stories
    // reach this point, so each candidate is judged on whether it belongs on THIS site before
    // it is written. Anything that fails stays a pending record (visible in the dashboard,
    // with its score and reason) but is never auto-published — an empty publishing day beats
    // an off-topic article. Disable per automation with ai_settings.relevance_gate = false.
    const gateEnabled = aiSettings.relevance_gate !== false
    const minRelevance = Number.isFinite(Number(aiSettings.min_relevance))
      ? Number(aiSettings.min_relevance)
      : DEFAULT_MIN_RELEVANCE
    const siteProfile = {
      name: automation.site_name,
      url: automation.site_url,
      keywords: automation.keywords,
      tags: automation.tags,
      audience: targetAudienceStr,
      extraContext: automation.extra_context,
    }

    const toAutoSchedule = []
    // Candidates judged this run that already exist in Airtable — their verdict is written
    // back so tomorrow's run reuses it instead of paying for the same judgement again.
    const freshVerdicts = []
    if (shouldAutoSchedule && runBudget > 0) {
      // Walking further than the budget lets a rejected story be replaced by the next one
      // down, but the walk is capped so an automation whose whole feed drifted off-topic
      // cannot spend a run's worth of API calls proving it.
      const maxChecks = Math.min(allCandidates.length, runBudget * 4 + 5)
      let checks = 0
      for (const cand of allCandidates) {
        if (toAutoSchedule.length >= runBudget) break
        if (!gateEnabled) { toAutoSchedule.push(cand); continue }

        // A backlog item already judged in an earlier run keeps its verdict — no second call.
        if (typeof cand.relevance_score === 'number' && cand.relevance_score >= 0) {
          if (cand.relevance_score >= minRelevance) toAutoSchedule.push(cand)
          else console.log(`[AUTO-PIPELINE] [${automation.name}] Off-topic (${cand.relevance_score}, cached): ${String(cand.title).substring(0, 60)}`)
          continue
        }

        if (checks >= maxChecks) break
        checks++
        const verdict = await assessRelevance(
          {
            title: cand.title,
            description: cand.description,
            content: cand.originalContent,
            source: cand.source,
          },
          siteProfile,
        ...
        if (!verdict.skipped) {
          cand.relevance_score = verdict.score
          cand.relevance_reason = verdict.reason
          freshVerdicts.push(cand)
        }
        if (verdict.publish) {
          toAutoSchedule.push(cand)
        } else {
          console.log(`[AUTO-PIPELINE] [${automation.name}] Not publishing (relevance ${verdict.score}/${minRelevance}): ${String(cand.title).substring(0, 60)} — ${verdict.reason}`)
        }
      }
    }

    const scheduledUrls = new Set(toAutoSchedule.map(a => a.url))
    const toPending = shouldAutoSchedule
      ? (instantPublish ? [] : allCandidates.filter(a => !scheduledUrls.has(a.url)))
      : allCandidates
    ...
    for (const cand of freshVerdicts) {
      if (!cand.id) continue  // brand-new items carry the verdict into their create payload
      try {
        await updateArticle(cand.id, {
          relevance_score: cand.relevance_score,
          relevance_reason: cand.relevance_reason || '',
        })
      } catch (err) {
        console.error(`[AUTO-PIPELINE] Could not store relevance for ${cand.id}:`, err.message)
      }
    }
    ...
    if (!fetchOnly) {
      ...
      for (const article of allCandidates) {
        ...
        if (currentContent.replace(/<[^>]+>/g, '').length < MIN_CONTENT_LENGTH && article.url) {
          try {
            const { content: scraped, links } = await scrapeArticleWithLinks(article.url)
            // Keep the source's own outbound links — they become the only URLs the
            // rewrite is allowed to cite (see lib/source-links.ts).
            article.sourceLinks = links
            if (scraped.length > currentContent.replace(/<[^>]+>/g, '').length) {
              article.originalContent = scraped
              console.log(`[AUTO-PIPELINE] [${automation.name}] Scraped full content for: ${article.title.substring(0, 50)} (${scraped.length} chars, ${links.length} links)`)
            }
          } catch (err) {
    ...
    // url → Airtable record id, so a report folded into another article can be marked
    // as merged. Seeded with what already exists and extended with records created below.
    const urlToRecordId = new Map(
      existingArticles.filter(a => a.automation_id === automation.id && a.url && a.id).map(a => [a.url, a.id])
    )
    ...
    if (toPending.length > 0) {
      ...
      try {
        const createdPending = await createArticlesBatch(pendingPayload)
        for (const a of toPending) existingUrls.add(a.url)
        // Remember the new record ids so a report merged into another article this run
        // can be marked in Airtable even though it was only just created.
        createdPending.forEach((rec, i) => {
          if (rec?.id && toPending[i]?.url) urlToRecordId.set(toPending[i].url, rec.id)
        })
      } catch (err) {
    ...
    const results = []
    // URLs already folded into another article this run, so one story cannot be merged
    // into two different articles.
    const mergedUrls = new Set()

    for (const article of toAutoSchedule) {
      if (mergedUrls.has(article.url)) {
        console.log(`[AUTO-PIPELINE] [${automation.name}] Skipping (already merged into another article): ${article.title.substring(0, 50)}`)
        continue
      }
      ...
      try {
        ...
        // Articles whose feed content was already long enough skipped the scrape above,
        // so they have no outbound links yet. Fetch them now — this runs only for the
        // handful of articles actually being rewritten, not the whole candidate pool.
        if (!article.sourceLinks && article.url) {
          try {
            const { links } = await scrapeArticleWithLinks(article.url)
            article.sourceLinks = links
          } catch {
            article.sourceLinks = []
          }
        ...
        // Several outlets reporting the same story become ONE article rather than
        // several thin ones. This is also what makes a longer article honest: three
        // reports carry enough material to write from, where one wire snippet did not.
        const siblingPool = allCandidates.filter(
          c => c.url && c.url !== article.url && !mergedUrls.has(c.url)
        )
        const related = findRelatedArticles(article, siblingPool)
        const relatedSources = []
        for (const sib of related) {
          let text = (sib.originalContent || sib.description || '').replace(/<[^>]+>/g, '').trim()
          try {
            const scraped = await scrapeArticleWithLinks(sib.url)
            if (scraped.content.length > text.length) text = scraped.content
            article.sourceLinks = [...(article.sourceLinks || []), ...scraped.links]
          } catch {}
          if (text.length < 150) continue
          relatedSources.push({ title: sib.title, url: sib.url, outlet: sib.source || '', content: text })
          mergedUrls.add(sib.url)
        }
        if (relatedSources.length) {
          console.log(`[AUTO-PIPELINE] [${automation.name}] Merging ${relatedSources.length} same-story report(s) into: ${article.title.substring(0, 50)}`)
        }
        ...
        await updateArticle(articleId, {
          ...
          // Refresh the card summary too — otherwise `description` keeps the original source
          // text (wrong language) while title/content are rewritten.
          ...((rewritten.meta_description || rewritten.subtitle) ? { description: rewritten.meta_description || rewritten.subtitle } : {}),
        ...
        // Record the merge so the folded-in reports are never published separately, in
        // this run or any later one.
        for (const src of relatedSources) {
          const recId = urlToRecordId.get(src.url)
          if (!recId) continue
          try {
            await updateArticle(recId, { merged_into: articleId })
          } catch (err) {
            console.error(`[AUTO-PIPELINE] Could not mark ${recId} as merged:`, err.message)
          }
        }
...
const payloads = toPush.filter(a => {
  ...
  // NOTE: deploy webhooks are NOT fired here anymore. Firing per hourly pipeline run caused
  // ~8 Netlify builds/day per site. Deploys are now throttled to once/day by the dedicated
  // /api/cron/deploy-sites cron. Content still lands via push each run; the daily build
  // regenerates the static/SEO pages. (The Netlify netlify.toml `ignore` trick did NOT work
  // for build-hook-triggered deploys, so throttling has to happen here at the source.)
