import { updateArticle, getArticles, getAutomation } from './airtable'
...
// Shared rewrite logic. Used by both the Next.js /api/articles/rewrite route
// (inline fallback) and the Netlify rewrite-background function (15-min timeout)
// so a rewrite reliably runs to completion and the result lands in Airtable.
export async function doRewrite(id, options, customInstructions) {
  ...
  // Collect the source's own outbound links: the rewrite may cite those and nothing else,
  // so it can't fabricate authoritative-looking URLs (see lib/source-links.ts).
  let sourceLinks = []
  if (article.url) {
    try {
      const scraped = await scrapeArticleWithLinks(article.url)
      sourceLinks = scraped.links
      console.log(`[rewrite] Found ${sourceLinks.length} citable source link(s)`)
    } catch {}
  }
